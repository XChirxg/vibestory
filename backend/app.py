"""
VibeStory Backend — app.py  v8.0
=================================
Changes from v7.0:
  - Removed all MongoDB training-data storage (yolo_training collection gone).
  - Labels go directly to the YOLO dataset folder on disk — that's it.
  - rename-label now takes the full corrected labels list (no DB lookup needed).
  - Simplified: one way to store data, one way to train.

Dataset lives at ./yolo_dataset/ — share it, zip it, train from it:
    python train.py
    GET /api/learn/export-dataset

MongoDB is still used only for: auth, stories, user scores.

Models (all LOCAL):
  STT    : openai-whisper "small"
  Images : nitrosocke/Ghibli-Diffusion
  TTS    : kokoro-onnx → WAV
  Detect : best.pt  ← your seniors' fine-tuned YOLO
